...
var m3u8downloader = function(masterPlayListURL, destination, callback) 
{
    ...
    function handleResponse(error, response, body) 
    {
        ...
        var dirName = fileMasterPlayList.replace('/' + masterResponse[masterResponse.length-1], '');
        try 
        {
        ...
        function afterWrite(err) 
        {
            ...
            function handleContent(part, asyncCb) 
            {
                ...
                try 
                {
                    if (part.indexOf("://") > -1) 
                    {
                        mkdirp(destination + "/" + masterResponse[1] + '/' + res2[res2.length - 2]);
                        mediaPlayListFile = part;
                    }
                    else 
                    {
                        mkdirp(destination + "/" + masterResponse[1] + '/' + res2[0]);
